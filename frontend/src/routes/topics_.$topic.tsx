...
export const Route = createFileRoute("/topics_/$topic")({
  validateSearch: (s: Record<string, unknown>): Search => {
    const c = typeof s.cluster === "string" ? s.cluster : "";
    return { cluster: c };
  },
...
function tabPath(id: string): TabPath {
  return `/topics/$topic/${id}` as TabPath;
}
      ...
      <nav className="flex items-center gap-1 border-b border-border">
        {TABS.map((tab) => (
          <Link
            key={tab.id}
            to={tabPath(tab.id)}
            params={{ topic } as never}
            search={{ cluster } as never}
...
function Breadcrumbs({ cluster, topic }: { cluster: string; topic: string }) {
  return (
    <div className="flex items-center gap-2 text-[11px] font-semibold uppercase tracking-wider text-muted">
      <span>{cluster || "—"}</span>
      <span>›</span>
      <Link to="/topics" search={{ cluster } as never} className="hover:text-text">
        Topics
      </Link>
      <span>›</span>
...
function TopicActions({
  ...
  const delMut = useMutation({
    ...
    onSuccess: () => {
      ...
      navigate({ to: "/topics", search: { cluster } as never });
    },
