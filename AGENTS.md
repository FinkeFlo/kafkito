...
- **Hard gate for frontend work:** before finishing any turn that touched `frontend/`, `cd frontend && bun run lint && bun run build && bun run check:palette && bun run check:strings && bun run check:tokens && bun run check:routes && bun run check:dates && bun run test` must all pass.
