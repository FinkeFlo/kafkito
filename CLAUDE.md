...
- **Hard gate for frontend work:** before finishing any turn that touched `frontend/`, `cd frontend && bun run lint && bun run build && bun run check:palette` must all pass. For visible UI work, also spot-check light + dark mode against the running dev server.
